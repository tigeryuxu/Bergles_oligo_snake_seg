...
#s_path = './Checkpoints/rate_e8_6_Bergles_single_oligo_512x512_RERUN/'
s_path = './Checkpoints/CLIP_e5_6_Bergles_single_oligo_512x512_RERUN/'
#s_path = './Checkpoints/CLIP_6_Bergles_single_oligo_512x512_RERUN/'
#s_path = './Checkpoints/CLIP_SPEED_e5_6_Bergles_single_oligo_512x512_RERUN/'

s_path = './Checkpoints/CLIP_SPEED_e5_6_Bergles_single_oligo_512x512_RERUN/'

s_path = './Checkpoints/8_SPATIAL_Bergles_single_oligo_512x512/'

#s_path = './Checkpoints/8_CLIP_SPATIAL_Bergles_single_oligo_512x512/'
input_path = '/scratch/yxu233/Training_data_Bergles_lab_512x512_DILATED/'

#input_path = '/lustre04/scratch/yxu233/Training_data_Bergles_lab_512x512/'

#s_path = './Checkpoints/6_Bergles_single_oligo_512x512/'
#input_path = './Input raw/resized_512x512/'
...
if len(onlyfiles_check) < 4:  
    ...
    with open(s_path + 'jaccard.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
        ...
        #plot_jaccard = plot_jaccard[0:3640]
    ...
    with open(s_path + 'jaccard_val.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
        ...
        #plot_jaccard_val = plot_jaccard_val[0:3640]
...
for P in range(8000000000000000000000):
    ...
    for i in range(len(input_counter)):
        ...
        truth_im, empty = load_class_truth_3D(truth_name, num_truth_class, input_size, depth, spatial_weight_bool=0)


        """ Load spatial_weights """
        spatial_W_name = examples[input_counter[i]]['spatial_weights']   
        weighted_labels, empty = load_class_truth_3D(spatial_W_name, num_truth_class, input_size, depth, spatial_weight_bool=0, splitter='SPATIAL_W', pick=1)
        ...
        if len(batch_x) == batch_size:
           ...
           if epochs % plot_every == 0:
              ...
              for batch_i in range(len(validation_counter)):
                  ...
                  truth_im_val, empty = load_class_truth_3D(truth_name, num_truth_class, input_size, depth, spatial_weight_bool=1)


                  """ Load spatial_weights """
                  spatial_W_name = examples[validation_counter[rand_idx]]['spatial_weights']   
                  weighted_labels_val, empty = load_class_truth_3D(spatial_W_name, num_truth_class, input_size, depth, spatial_weight_bool=0, splitter='SPATIAL_W', pick=1)
              ...
              if epochs > 500:
                  if epochs % 100 == 0:
                      ...
                      #plt.subplot(333); plt.imshow(np.asarray(input_im, dtype = np.uint8)); plt.title('Input');
                      plt.subplot(333); plt.imshow(weighted_labels[:, :, 2]); plt.title('weighted');    #plt.pause(0.005)
                      plt.subplot(336); plt.imshow(input_im[:, :, 1]); plt.title('Cell mask');
                      plt.subplot(339); plt.imshow(raw_truth[:, :, 2]); plt.title('Ch2: truth');       
