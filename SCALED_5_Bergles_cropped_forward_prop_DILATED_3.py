...
"""  Network Begins:
"""
s_path = './Checkpoints/SCALED_5_Bergles_cropped_forward_prop_DILATED_2/'

input_path = '/scratch/yxu233/TRAINING FORWARD PROP SCALED FINISHED/'
...
for P in range(8000000000000000000000):
    ...
    for i in range(len(input_counter)):
        ...
        if rand_num <= 5:     # 30% of the time want one with NO splits
             rand_num = randint(0, 10)
        ...
        """ dilate truth and seed """
        dil_size = 2
        truth_skel = skeletonize_3d(truth_im[:, :, :, 1]);  truth_skel[truth_skel > 0] = 1
        truth_class_dil = dilate_by_ball_to_binary(truth_skel, radius=dil_size)     
        
        sp_weighted_labels = spatial_weight(truth_skel,edgeFalloff=10,background=0.01,approximate=True)
        weighted_labels[:, :, :, 1] = sp_weighted_labels
        truth_im[:, :, :, 1] = truth_class_dil
        
        # make background as well:
        
        background = np.copy(truth_class_dil)
        ...
        test = skeletonize_3d(seed_crop_mask);  test[test > 0] = 1
        test = dilate_by_ball_to_binary(test, radius=dil_size)         
        seed_crop_mask = test  
           ...
           if epochs % plot_every == 0:
              ...
              for batch_i in range(len(validation_counter)):
                  ...
                  truth_im_val, weighted_labels_val = load_class_truth_3D(truth_name, num_truth_class, width_max=input_size, height_max=input_size, depth=depth, 
                                                                          spatial_weight_bool=0,  delete_seed=seed_crop_mask_val, skip_class=2, resized_check=1)
          
          
                  truth_skel_val = skeletonize_3d(truth_im_val[:, :, :, 1]);  truth_skel_val[truth_skel_val > 0] = 1
                  truth_dil_val = dilate_by_ball_to_binary(truth_skel_val, radius=dil_size)          
                  
                  sp_weighted_labels = spatial_weight(truth_skel_val,edgeFalloff=10,background=0.01,approximate=True)
                  weighted_labels_val[:, :, :, 1] = sp_weighted_labels
                  truth_im_val[:, :, :, 1] = truth_dil_val
                  
                  # make background as well:
                  
                  background = np.copy(truth_dil_val)
                  background[background > 0] = 2
                  background[background == 0] = 1
                  background[background > 1] = 0
                  truth_im_val[:, :, :, 0] = background
                  
          
                  seed_crop_mask_val[seed_crop_mask_val > 0] = 1
                  test = skeletonize_3d(seed_crop_mask_val);  test[test > 0] = 1
                  test = dilate_by_ball_to_binary(test, radius=dil_size)         
                  seed_crop_mask_val = test
