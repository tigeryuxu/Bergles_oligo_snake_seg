...
for i in range(len(examples)):              
        ...
        for tree in all_trees:
             ...
             while np.asarray(tree.visited.isnull()).any():   
                  ...
                  clf = NearestNeighbors(n_neighbors=2).fit(cur_coords)
                  ...
                  cur_coords = cur_coords[order]   # SORT BY ORDER
                  
                  
                  ### Step size:
                  step_size = 10
                  step_size_first = step_size
                  tot_steps = math.ceil(len(cur_coords)/10)
                  ...
                  for step in range(step_size_first, len(cur_coords), step_size):    
                      ...
                      plot_save_max_project(fig_num=5, im=crop_seed, max_proj_axis=-1, title='crop seed dilated', 
                                            name=s_path + filename + '_Crop_'  + str(num_tree) + '_' + str(iterator) + '_step_' + str(step) +  '_seed.png', pause_time=0.001)
                      plot_save_max_project(fig_num=2, im=output_PYTORCH, max_proj_axis=-1, title='segmentation', 
                                            name=s_path + filename + '_Crop_'   + str(num_tree) + '_' + str(iterator) + '_step_' + str(step) +  '_segmentation.png', pause_time=0.001)
                      plot_save_max_project(fig_num=3, im=crop, max_proj_axis=-1, title='input', 
                                            name=s_path + filename + '_Crop_'   + str(num_tree) + '_' + str(iterator) + '_step_' + str(step) +  '_input_im.png', pause_time=0.001)
                  ...
                  # output_PYTORCH, box_x_min, box_x_max, box_y_min, box_y_max, box_z_min, box_z_max, boundaries_crop = crop_around_centroid_with_pads(output_tracker, y, x, z, crop_size, z_size, height_tmp, width_tmp, depth_tmp)
                      ...
                      for seg in all_seg:
                          ...
                          for be in cc_be:
                              ...
                              if (cur_seg[:, None] == cur_be).all(-1).any():
                                  ...
                                  ### expand the cur_be
                                  neighborhood_be = expand_coord_to_neighborhood(cur_be, lower=2, upper=3)
