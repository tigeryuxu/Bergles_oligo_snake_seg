...
#SBATCH --time=08:00:00 # time (DD-HH:MM) OR (hh:mm:ss) format
#SBATCH --output=%N-%j.out # %N for node name, %j for jobID

module load cuda cudnn python/3.5   # load python and then tensorflow
source tensorflow/bin/activate
python ./SCALED_5_Bergles_cropped_forward_prop_DILATED_3.py --variable_update=parameter_server --local_parameter_device=cpu  # script to run (change to Myelin UNet)
