...
"""
#s_path = './Checkpoints/'
s_path = './Checkpoints/4_Bergles_single_oligo_RESIZED_deep_net_FLOAT32_RERUN/'


input_path = '/scratch/yxu233/Training_data_Bergles_lab_960x960/'
...
images = glob.glob(os.path.join(input_path,'*input_960x960_RESIZED.tif'))
natsort_key1 = natsort_keygen(key = lambda y: y.lower())      # natural sorting order
images.sort(key = natsort_key1)
examples = [dict(input=i,truth=i.replace('input_960x960_RESIZED','truth'), cell_mask=i.replace('input_960x960_RESIZED','input_cellMASK_960x960_RESIZED')) for i in images]
           ...
           if epochs % plot_every == 0:
              ...
              if epochs > 500:
                  if epochs % 100 == 0:
                      ...
                      plt.subplot(336); plt.imshow(raw_truth[:, :, 0]); plt.title('Ch1: background');
